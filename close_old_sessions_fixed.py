#!/usr/bin/env python3
"""
Скрипт для закрытия старых незакрытых сессий (ИСПРАВЛЕННАЯ ВЕРСИЯ)
Умно определяет время закрытия на основе времени входа
"""
import asyncio
from datetime import datetime, time, timedelta
from sqlalchemy import select, and_
from app.database import AsyncSessionLocal
from app import models

async def close_old_sessions():
    async with AsyncSessionLocal() as db:
        print('=' * 90)
        print('ЗАКРЫТИЕ СТАРЫХ НЕЗАКРЫТЫХ СЕССИЙ (ИСПРАВЛЕННАЯ ВЕРСИЯ)')
        print('=' * 90)
        print()
        
        # Сначала удаляем ошибочные события с отрицательной длительностью
        result = await db.execute(
            select(models.AttendanceEvent)
            .filter(
                models.AttendanceEvent.event_type == 'exit',
                models.AttendanceEvent.event_type_description == 'Auto-closed by system'
            )
        )
        auto_exits = result.scalars().all()
        
        deleted_count = 0
        for exit_event in auto_exits:
            # Проверяем, есть ли соответствующий вход
            result = await db.execute(
                select(models.AttendanceEvent)
                .filter(
                    and_(
                        models.AttendanceEvent.user_id == exit_event.user_id,
                        models.AttendanceEvent.event_type == 'entry',
                        models.AttendanceEvent.timestamp < exit_event.timestamp
                    )
                )
                .order_by(models.AttendanceEvent.timestamp.desc())
                .limit(1)
            )
            entry_event = result.scalar_one_or_none()
            
            if entry_event and entry_event.timestamp > exit_event.timestamp:
                print(f'⚠️  Найден некорректный выход:')
                print(f'   Вход: {entry_event.timestamp}')
                print(f'   Выход: {exit_event.timestamp}')
                print(f'   ❌ Удаляем некорректное событие')
                await db.delete(exit_event)
                deleted_count += 1
        
        if deleted_count > 0:
            await db.commit()
            print(f'🗑️  Удалено некорректных событий: {deleted_count}')
            print()
        
        # Теперь закрываем незакрытые сессии ПРАВИЛЬНО
        result = await db.execute(select(models.User))
        users = result.scalars().all()
        
        closed_count = 0
        
        for user in users:
            # Получаем последнее событие пользователя
            result = await db.execute(
                select(models.AttendanceEvent)
                .filter(models.AttendanceEvent.user_id == user.id)
                .order_by(models.AttendanceEvent.timestamp.desc())
                .limit(1)
            )
            last_event = result.scalar_one_or_none()
            
            if last_event and last_event.event_type == 'entry':
                days_ago = (datetime.now() - last_event.timestamp.replace(tzinfo=None)).days
                
                # Закрываем только если сессия старше 1 дня
                if days_ago >= 1:
                    name = user.full_name or user.hikvision_id or f'User#{user.id}'
                    entry_ts = last_event.timestamp.strftime("%Y-%m-%d %H:%M:%S")
                    entry_hour = last_event.timestamp.hour
                    
                    print(f'🔍 Найдена незакрытая сессия:')
                    print(f'   Пользователь: {name}')
                    print(f'   Вход: {entry_ts} ({days_ago} дн. назад)')
                    
                    # Умное определение времени выхода
                    if entry_hour < 12:
                        # Утренний вход - закрываем вечером того же дня (18:00)
                        exit_time = last_event.timestamp.replace(
                            hour=18, minute=0, second=0, microsecond=0
                        )
                    else:
                        # Дневной/вечерний вход - закрываем через 8 часов
                        exit_time = last_event.timestamp + timedelta(hours=8)
                        # Но не позже 23:59 того же дня
                        end_of_day = last_event.timestamp.replace(
                            hour=23, minute=59, second=59, microsecond=0
                        )
                        if exit_time > end_of_day:
                            exit_time = end_of_day
                    
                    # Создаем событие выхода
                    exit_event = models.AttendanceEvent(
                        user_id=user.id,
                        employee_no=last_event.employee_no,
                        name=last_event.name,
                        event_type='exit',
                        event_type_description='Auto-closed by system (fixed)',
                        timestamp=exit_time,
                        terminal_ip=last_event.terminal_ip,
                        card_no=last_event.card_no
                    )
                    
                    db.add(exit_event)
                    closed_count += 1
                    
                    exit_ts = exit_time.strftime("%Y-%m-%d %H:%M:%S")
                    duration = (exit_time - last_event.timestamp).total_seconds() / 3600
                    print(f'   ✅ Создано событие выхода: {exit_ts}')
                    print(f'   ⏱️  Длительность сессии: {duration:.1f} часов')
                    print()
        
        if closed_count > 0:
            await db.commit()
            print('=' * 90)
            print(f'✅ Закрыто сессий: {closed_count}')
            print('=' * 90)
        else:
            print('✅ Нет старых незакрытых сессий для закрытия')
            print('=' * 90)

asyncio.run(close_old_sessions())

